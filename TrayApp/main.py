...
def toggle_float_window():
    global float_window, float_thread
    if float_window and float_window.winfo_exists():
        ...
        if not float_thread or not float_thread.is_alive():
            float_thread = threading.Thread(target=run_float_window, daemon=True)
            float_thread.start()
